...
\footnote{Comment to Alan: I was always wondering how $y.e$, i.e., a delimited continuation here is very similar to blocking communication + guarded commands in CPS.}
...
\caption{The interesting subset of rules of Eff's small step operational semantics}
\label{fig:mini-semantics}
\end{figure}
...
\begin{table}
  ...
  \hline \hline
  \textsc{Let-Val} & 
    If a computation \emph{returns} an expression $e$, it is bound to the identifier $x$ in the computation $c$. \\ \hline
  \textsc{Let-Effect} & 
  ...
  \textsc{Perform} &
    ...
    effect package might not always be possible. \\ \hline
  \textsc{Handle-Val} & 
  ...
  \textsc{Handle-Eff-Match} &
    ...
    The argument of the effect is bound to $x$ and the delimited continuation $(y.c)$ is bound to $k$. \\ \hline
...
\begin{figure}
  ...
  \caption{The interesting subset of type checking rules for Eff expressions and computations}
...
\begin{table}
  ...
  \hline \hline
  \textsc{T-Val} &
    This rule is standard. The expression $e$ and the computation $\effVal e$ are given the same type.\\ \hline
  \textsc{T-Perform} & This rule is similar to function application for effects.\\ \hline
  \textsc{T-Handler} & The rule for typing handlers might be the scariest one. We are saying here that
    ...
    is applied.\\ \hline
  \textsc{T-WithHandle} & As applying handlers happens with with-handle blocks this rule is similar to
    the function application typing too. \\ \hline
  \end{tabular}
...
\begin{lstlisting}[language=efflang]
  ...
  if true then absurd (raise (Error "Lie detected!")) else 1337;;
\end{lstlisting}
