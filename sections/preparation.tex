...
\begin{figure}[hbt!]
  ...
  string and a result of type unit.}, label=effect-perform-keywords, language=efflang]{../code_examples/first_program.eff}
\end{figure}
...
An effect rule is a generalisation of SML's \cite{milner1997definition} \verb|handle| or Java's \verb|catch|. It differs from it in
...
\subsection{CPS and Scheme's call/cc}

Continuations are everywhere, consider the following code:
...
A similar piece of code can be written with call/cc:
\begin{lstlisting}[language=scheme]
...
the program. They represent the continuation of the computation enclosed by a
with-handle block.
...
in 42;;
\end{lstlisting}
In the code above \verb|(r,s)| takes the values \verb|((), "Hello World")| rather than
\verb|(42, "Hello World")| one would expect with Scheme's call/cc. The fact that continuations
