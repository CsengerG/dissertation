...
of Eff, which is a with-handle block. However, to do this, we need to be
...
\mylisting{efflang}
{The Hello World example in Eff. The code above evaluates to the pair \texttt{((), "Hello World!")}.}
{hello-world}
...
\lstinline{()} value in \autoref{handler-keyword}---\lstinline{Print} is of type $\mathtt{string} \tto \mathtt{unit}$).
...
Kelvin but we need this in Fahrenheit to display it on a user interface: TODO
...
from. This also allows us to resume them more than once if necessary. Hence we can think about delimited continuations simply as functions.
...
\begin{myfigure}[1]
  {The interesting subset of rules of Eff's small step operational semantics}
  {fig:mini-semantics}
  $$
  \begin{prooftree}
...
all \emph{effect signatures} of the form $E_i : A_i \tto B_i$.

\begin{myfigure}[1]
{The interesting subset of type checking rules for Eff expressions and computations}
...
Again, explanation of these rules is given in \autoref{tab:typing-rule-explanation} in a similar way to the semantics.
\begin{table}
  \centering
  {\renewcommand{\arraystretch}{1.3}
  \rowcolors{2}{gray!17}{white}
