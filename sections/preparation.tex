...
Here I state the syntax of core Eff:
...
The operational semantics of core Eff:
\begin{figure}
$$
\begin{prooftree}
\Hypo{}
\Infer1{\effIf \effTrue \effThen c_1 \effElse c_2 \leadsto c_1}
\end{prooftree}
$$

\caption{Small step operational semantics of core Eff}
\end{figure}
The denotational semantics of core Eff:
\begin{figure}
\caption{Denotational semantics of core Eff}
\end{figure}
...
Always having an \emph{automated} test suite at hand avoids trial and error testing which is both error prone and
time consuming.
...
that the code I have written so far is correct. Other advantages were the ability to notice
quickly if a code change broke some already implemented feature and the ability to refactor
code with confidence (which I had to do frequently---from internships I knew that my coding style
is such that this will be unavoidable if I want to work with a maintainable codebase).

Third, my success criterion was a correct working implementation of a compiler so tests were crucial for the
success of my project as well.
